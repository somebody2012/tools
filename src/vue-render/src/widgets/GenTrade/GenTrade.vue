...
export default {
  ...
  methods:{
    saveTmpExcel(){
      let dataArr = [
        {"组名称(trade-group)":"","栏位名称":"","必输(Q)|不可用(D)|只读(R)|不可见(V)":"","sync字段竖线'|'分割":""},
        {"组名称(trade-group)":"","栏位名称":"","必输(Q)|不可用(D)|只读(R)|不可见(V)":"","sync字段竖线'|'分割":""},
        {"组名称(trade-group)":"","栏位名称":"提交","必输(Q)|不可用(D)|只读(R)|不可见(V)":"","sync字段竖线'|'分割":""},
      ];
    ...
    async gentTradeMenu(){
      ...
      let sql1 = `INSERT INTO \`pub_db\`.\`IB_UPM_PRIVLEGES_RLT\`(\`PRVLG_ID\`, \`APP_NO\`, \`MENU_VIEW_NO\`, \`ENTITY_TYP_CD\`, \`ENTITY_NO\`, \`PARE_RANK_PRVLG_NO\`, \`MATN_ORG_NO\`, \`MATN_TELR_NO\`, \`MATN_DT\`, \`MATN_TM\`) VALUES ('${MENU_NO}', 'TE', '${MENU_VIEW_NO}', 'T', '${MENU_NO}', 'TER023', '0000', '000000', '2019-10-22', '16:26:08');`
      let sql2 = `INSERT INTO \`pub_db\`.\`IB_UPM_PRIVLEGES_RLT\`(\`PRVLG_ID\`, \`APP_NO\`, \`MENU_VIEW_NO\`, \`ENTITY_TYP_CD\`, \`ENTITY_NO\`, \`PARE_RANK_PRVLG_NO\`, \`MATN_ORG_NO\`, \`MATN_TELR_NO\`, \`MATN_DT\`, \`MATN_TM\`) VALUES ('${MENU_NO}', 'TE', '${MENU_VIEW_NO}', 'T', '${MENU_NO}', 'TETG00022', '0000', '000000', '2019-10-22', '16:25:07');`;
